import pytest
from src.domain.models.post_models_domain import PostModelUpdateDomain
from src.presentation.errors.exception_custom_errors_presentation import (
    ExceptionCustomPresentation,
)
from src.use_case.implements.post.tests.setup import repository, use_case, data, user

new_data = PostModelUpdateDomain(**data)
...
class TestUpdatePostUseCase:
    ...
    async def test_update_post_post_id_not_found(self):
        ...
        result = await use_case.execute(data=new_data, user=user, post_id=new_data.id)
    ...
    async def test_update_post_use_case_success(self):
        ...
        assert result.body == {"id": 1, "html": new_data.html, "user_id": 1}
