...
sqlite_db = os.path.abspath(os.path.join(current_dir, "../sqlite.db"))

env = PydanticEnv().env
if env == "dev":
    config.set_main_option("sqlalchemy.url", PydanticEnv().database_url)
elif env == "test":
    config.set_main_option("DATABASE_URL", f"sqlite:///{sqlite_db}")
